    ...
    return (
      <div className="min-h-screen bg-background-secondary p-4 md:p-8 flex items-center justify-center">
        <div className="bg-surface border border-border rounded-surface shadow-elevation-4 p-8 max-w-md w-full text-center">
          ...
          <p className="text-text-secondary mb-4">{submitError}</p>
          <p className="text-body-sm text-text-secondary mb-8">Product ID: {params?.id || "Not available"}</p>
          <div className="space-y-3">
            <Link href="/admin/products" className="block w-full bg-primary text-primary-foreground py-3 rounded-control font-semibold hover:bg-primary-hover transition-colors">
              Back to Products
            </Link>
            <Button type="button" variant="outline" size="lg" onClick={() => params?.id && fetchProduct(params.id)} className="w-full font-semibold">
              Try Again
            </Button>
          </div>
        </div>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6 bg-background-secondary">
        <div className="bg-surface border border-border rounded-surface shadow-elevation-4 p-8 max-w-md w-full text-center">
          ...
          <h2 className="text-h3 font-bold text-text-primary mb-3">Successfully Updated!</h2>
          <p className="text-text-secondary mb-8">Your product has been updated. Redirecting back to products list...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-surface rounded-surface shadow-elevation-3 border border-border-light overflow-hidden">
          <div className="border-b border-border-light bg-background-secondary/50 px-8 py-6">
            <h1 className="text-h3 font-bold text-text-primary">Edit Product</h1>
            <p className="text-text-secondary mt-1">Update your product's details and inventory.</p>
          </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-surface rounded-surface border border-border">
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <Input
                          ...
                          className={`pl-8 ${errors.price ? 'bg-destructive-background' : ''}`}
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        className={errors.stock ? 'bg-destructive-background' : ''}
                        min="0" placeholder="0"
                      />
                  ...
                  <div className="space-y-6">
                    <div className="flex flex-wrap gap-4 p-4 bg-surface rounded-surface border border-border">
                      ...
                      {hasSizes && (
                        <div className="ml-auto flex items-center bg-background-secondary rounded-control border border-border p-1">
                          <label className={`cursor-pointer px-3 py-1 rounded-control text-caption-md font-medium transition-colors ${sizingType === 'size' ? 'bg-primary/10 text-primary' : 'text-text-secondary hover:bg-surface-hover'}`}>
                            <input type="radio" value="size" checked={sizingType === 'size'} onChange={() => setSizingType('size')} className="sr-only" /> Use Sizes (S, M, L)
                          </label>
                          <label className={`cursor-pointer px-3 py-1 rounded-control text-caption-md font-medium transition-colors ${sizingType === 'age' ? 'bg-primary/10 text-primary' : 'text-text-secondary hover:bg-surface-hover'}`}>
                    ...
                    <div className="space-y-4">
                      {variants.map((variant, vIdx) => (
                        <div key={vIdx} className="border border-primary/20 bg-primary/5 rounded-surface p-5 relative">
                          {variants.length > 1 && (
                            <button
                              type="button"
                ...
                <div className={`border-2 border-dashed rounded-surface p-8 transition-colors ${images.length === 0 ? 'border-border-strong bg-background-secondary hover:bg-background-tertiary' : 'border-border'}`}>
                  {images.length === 0 ? (
                    <div className="text-center">
                      <div className="w-16 h-16 bg-surface border border-border rounded-overlay flex items-center justify-center mx-auto mb-4 shadow-elevation-1">
                      ...
                      <label
                        ...
                        className="inline-flex items-center gap-2 px-6 py-2.5 bg-primary text-primary-foreground rounded-surface font-semibold hover:bg-primary-hover cursor-pointer shadow-elevation-1 transition-all"
                      >
                    ...
                    <div className="space-y-6">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          ...
                          <div key={index} className="flex flex-col h-full justify-end gap-2">
                            ...
                            {uniqueColorsArray.length > 0 && (
                              <select
                                ...
                                className={`w-full text-caption-md py-2 px-2 rounded-control border-2 transition-colors ${image.assignedColor ? 'border-primary bg-primary/10 text-primary font-bold' : 'border-border bg-surface text-text-primary font-medium'}`}
                              >
                        ...
                        <label htmlFor="image-upload" className="self-end w-full flex flex-col items-center justify-center aspect-square border-2 border-dashed border-border-strong rounded-surface cursor-pointer hover:bg-surface-hover hover:border-primary/40 transition-colors group">
