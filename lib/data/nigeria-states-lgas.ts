/**
 * COMMERCE layer — reference data: Nigeria's 36 states + FCT and their official
 * Local Government Areas (774 total). Powers the State -> LGA cascading pickers
 * in checkout and the admin shipping zone form (lib/commerce/shipping-match.ts).
 *
 * Single flat dataset consumed as a whole (state key -> full LGA list) — kept
 * as one file rather than split per-state, which would fragment something
 * that's always read together, per CLAUDE.md's file-size exception for cases
 * where splitting would hurt readability. Compiled from general knowledge of
 * Nigeria's administrative geography, not fetched from an official registry —
 * treat as a good-faith reference and correct any misnamed/missing LGA you spot.
 */
...
export const NIGERIA_STATES_LGAS: Record<string, string[]> = {
  ...
  // Keyed 'Abuja' (not the official 'FCT') to match this app's existing seeded
  // zone data and UI copy, which already refer to the Federal Capital Territory
  // as 'Abuja' throughout.
  'Abuja': [
...
export const NIGERIA_STATE_NAMES = Object.keys(NIGERIA_STATES_LGAS);
